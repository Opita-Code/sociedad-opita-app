 ...
 */
function docMentionsPersona(doc: CorpusDoc, variants: string[], personaId?: string): boolean {
...
describe("golden retrieval queries — expanded (Q5..Q10)", () => {
  it("Q5: 'Don Eliécer, ¿qué opina de la política?' → persona bound in top-K", async () => {
    const results = await topK("Don Eliécer opinión política patrón finca");
    expect(
      topKBindsPersona(results, ["don eliecer", "don-eliecer", "don_eliecer"], "don_eliecer_patron")
    ).toBe(true);
  }, 30_000);

  it("Q6: 'Jhon Fredy, ¿por qué volviste a Tello?' → persona bound in top-K", async () => {
    const results = await topK("Jhon Fredy joven volvió retornado migración");
    expect(
      topKBindsPersona(results, ["jhon fredy", "jhon-fredy", "jhon_fredy"], "jhon_fredy_joven")
    ).toBe(true);
  }, 30_000);

  it("Q7: 'Doña Prudencia, ¿cuál es su mayor preocupación?' → persona bound in top-K", async () => {
  ...
  it("Q8: 'Don Octavio, ¿cómo talla la madera?' → persona bound in top-K", async () => {
    ...
    expect(
      topKBindsPersona(results, ["don octavio", "don-octavio", "don_octavio"], "don_octavio_medico")
    ).toBe(true);
  }, 30_000);

  it("Q9: 'Cuéntame de la plaza del pueblo' → top doc mentions plaza / central", async () => {
  ...
  it("Q10: 'Háblame del colegio de Tello' → top doc mentions colegio / escuela", async () => {
  ...
  it("Q5..Q10 — every expanded query returns a non-empty top-K with positive score", async () => {
  ...
  it("Q5..Q10 — top-K returns exactly 4 docs by default, sorted descending", async () => {
