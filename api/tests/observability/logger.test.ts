...
async function captureLogAsync<T>(
  fn: () => Promise<T>
): Promise<{ lines: CapturedLine[]; result: T }> {
  const spy = vi.spyOn(console, "log").mockImplementation(() => {});
  try {
...
describe("logger — JSON envelope", () => {
  it("info() emits one JSON line with ts, level, event, context", () => {
    ...
    // `noUncheckedIndexedAccess` is satisfied — `lines[0]` is typed
...
describe("logger — PII redaction", () => {
  it("redacts email addresses to [REDACTED_EMAIL]", () => {
    const { lines } = captureLog(() => logger.info("evt.contact", { email: "juan@example.com" }));
  ...
  it("redacts emails embedded in longer strings", () => {
    ...
    expect((lines[0]!.context as { msg: string }).msg).toBe("Contactame a [REDACTED_EMAIL] pronto");
  });
  ...
  it("redacts international phone numbers to [REDACTED_PHONE]", () => {
    const { lines } = captureLog(() => logger.info("evt.call", { phone: "+57 312 612 6085" }));
    expect((lines[0]!.context as { phone: string }).phone).toBe("[REDACTED_PHONE]");
  });

  it("redacts phone numbers written with dots or dashes", () => {
    const { lines } = captureLog(() => logger.info("evt.call2", { phone: "+1-415-555-0199" }));
    expect((lines[0]!.context as { phone: string }).phone).toBe("[REDACTED_PHONE]");
  });
  ...
  it("does not redact when there is no PII", () => {
    const { lines } = captureLog(() => logger.info("evt.safe", { msg: "Hola, ¿cómo va todo?" }));
